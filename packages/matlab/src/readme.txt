    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_canary_batch.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * canary_batch.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * canary_batch.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * canary_batch (application created by MATLAB Compiler)
        * run_canary_batch.sh (shell script that can be run to 
    ...
    All Platforms:
        * canary_batch.dll (shared library, file extension varies by 
          platform)
        * canary_batch.h (library header file)
        * canary_batch.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * canary_batch.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * canary_batch.xml (documentation files)
        * canary_batch.pdb (program database file - if DEBUG option 
          selected)
        * canary_batch.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * canary_batch.ctf (Component Technology File archive, contains 
          compiled M-code)
        * canary_batch_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * canary_batch.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * canary_batch.ctf (Component Technology File archive, contains 
          compiled M-code)
        * canary_batch.xla (Optionally created and placed in
          C:\Program Files\cygwin\home\dbhart\Canary\trunk\source\intermediate directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
